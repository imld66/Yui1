...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\ Bem vindo(a) ao grupo *${mdata.subject}* Yui que manda!`
				...
				teks = `Adeus. @${num.split('@')[0]} Não aguentou `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭Yui trabalhando ❬❗❭',
				success: '️❬ ✔ ❭ PRONTO PAPA',
				error: {
				...
				only: {
					...
					Badmin: '❬❗❭O Preciso de adm papa ;3 '
				}
			}
...
case 'lofi':
            ...
            case 'criador':
            case 'keke':
                  client.sendMessage(from, {displayname: "LD adm", vcard: vcard}, MessageType.contact, { quoted: mek})
